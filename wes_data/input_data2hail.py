...
hl.init(spark_conf={'spark.driver.memory': '700g'})

#region import merged vcf file

# hl.import_vcf('/users/qwu24/data/silvio/SPARK_Data_Aug_2021/pub/WES1/Variants/DeepVariant/wes1_27281_exome.deepvariant.vcf.gz', reference_genome='GRCh38', \
...
# 	force_bgz=True, array_elements_required=False).write('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.mt', overwrite=True)
mt = hl.read_matrix_table('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.mt')
...
# # export sample id
# mt.s.export("/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.sample_id.tsv")
...
# mt.filter_cols(mt.pheno.is_case == True).count_cols() # 9789
# mt.filter_cols(mt.pheno.is_case == False).count_cols() # 17492
# mt.filter_cols(hl.is_defined(mt.pheno.is_case)).count_cols() # 27281

# # count of is_female 
# mt.filter_cols(mt.pheno.is_female == True).count_cols() # 11924
# mt.filter_cols(mt.pheno.is_female == False).count_cols() # 15357
# mt.filter_cols(hl.is_defined(mt.pheno.is_female)).count_cols() # 27281
...
# hl.export_vcf(mt_1to5, '/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.individual_1to5.vcf.bgz')

# # vep annotated 1to5
# # import
# hl.import_vcf('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.individual_1to5.vep_anno_segdup.vcf', reference_genome='GRCh38', \
# 	contig_recoding={'1': 'chr1', '2': 'chr2', '3': 'chr3', '4': 'chr4', '5': 'chr5', '6': 'chr6', '7': 'chr7', '8': 'chr8', '9': 'chr9', '10': 'chr10', \
# 					'11': 'chr11', '12': 'chr12', '13': 'chr13', '14': 'chr14', '15': 'chr15', '16': 'chr16', '17': 'chr17', '18': 'chr18', '19': 'chr19', '20': 'chr20', \
# 					'21': 'chr21', '22': 'chr22', 'X': 'chrX', 'Y': 'chrY'}, \
# 	force_bgz=True, find_replace=('nul', '.'), array_elements_required=False).write('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.individual_1to5.vep_anno_segdup.mt', overwrite=True)
# mt_1to5_vep = hl.read_matrix_table('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.individual_1to5.vep_anno_segdup.mt')
# # shape of the matrix table
...
# mt_1to5_vep.info.CSQ[0].export('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.individual_1to5.CSQ.tsv')
...
vep_anno = hl.import_table('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.segdup.tsv', impute=True)
...
# # Removed variants on segment duplicated region, 4549169 variants remain.

# # filter not monoallelic
# print(mt_pass_vep_segdup.aggregate_rows(hl.agg.counter(mt_pass_vep_segdup.filters)))
# # frozendict({frozenset({'MONOALLELIC'}): 11971, None: 4537198})
...
# # Filtering 2.54% entries out of downstream analysis.
...
# # Filtering 0.49% entries out of downstream analysis.
...
# # Sample QC stats: Struct(mean=0.9696631762716488, stdev=0.016718991048648806, min=0.8759487683808377, max=0.9963686839322419, n=27281, sum=26453.38111186685)
...
# # # After filter, 27266 samples remain.
...
# # After filter, 4276376 variants remain.

# export to new matrix table
mt_pass_varQC.write('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.pass_QC.mt', overwrite=True)
...
mt_pass_QC = hl.read_matrix_table('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.pass_QC.mt')
# shape of the matrix table
mt_pass_QC.count()
# (4276376, 27266)
...
# de novo:  frozendict({'HIGH': 19026, 'LOW': 5301, 'MEDIUM': 2359})
...
# 	).export('/users/qwu24/data/silvio/Qing_Wu/SFARI/hail_out/SPARK/wes1_27281_exome.deepvariant.dnvs.tsv')
