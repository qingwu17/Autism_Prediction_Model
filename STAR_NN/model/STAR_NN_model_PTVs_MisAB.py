...
class STAR_NN(BaseEstimator):
    ...
    def __init__(self, model_params):
        # self.get_compiled_model = get_compiled_model
        ...
        # self.prediction_output = model_params['fitting_params']['prediction_output']
        # self.monitor = model_params['fitting_params']['monitor']
        # self.feature_importance = model_params['feature_importance']
        
    def fit(self, X_train, y_train, X_val, y_val, X_test, y_test, col_name_w_variant_type, random_seed):
        
        # print("\nIn sparse_mlp_model.Model.fit(), model: ")
        ...
        print("self.model=model, model=self.get_comppiled_model")
        self.model = model
        ...
        # variant_level_weight_df_filename = "/users/qwu24/data/silvio/Qing_Wu/SFARI/batch_jobs/python_script/net/wes12.deepvariant.selFeat1489.variant_level_weight." + str(random_seed) + ".csv"
        ...
        # gene_level_weight_df_filename = "/users/qwu24/data/silvio/Qing_Wu/SFARI/batch_jobs/python_script/net/wes12.deepvariant.selFeat1489.gene_level_weight." + str(random_seed) + ".csv"
...
class Sparse_mapping(Layer):
    ...
    def build(self, input_shape):
        ...
        # variants_order = np.arange(input_dimension) # 27687
        # gene_by3_order = np.repeat(np.arange(self.units), self.n_inputs_per_node) 
        # self.nonzero_idx = np.column_stack((variants_order, gene_by3_order))
...
def get_compiled_model(n_features, col_name_w_variant_type, sparse, sparse_first_layer, activation, use_bias, kernel_initializer, w_reg):
    ...
    # print("\nIn build_models.get_compiled_model()")
