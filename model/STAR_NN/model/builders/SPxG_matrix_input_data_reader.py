...
def read_SPxG_matrix_table(data_path, SPxis_case_is_female_header_filename, GxSP_matrix_filename, var2gene_unconsec_lst_filename, var2gene_lst_filename, idx_sp2exclude_filename, PGS_filename, iWESv1_subset=False):
    ...
    print("Shape of input data wo SP_id:", input_df.shape) 

    # get input data(x), labels, sample_name, genes 
    labels = input_df['is_case']
    # X = input_df.drop(['is_case', 'PGS', 'is_female'], axis=1)
    X = input_df.drop(['is_case'], axis=1)

    col_name_X = X.columns

    # # match with input selected genes list
    # if selected_genes is not None:
    #     intersect = set.intersection(set(col_name_X), selected_genes)
    #     X = X.loc[:, intersect]
    #     col_name_X = intersect

    return X, labels, samples_SPID

def filter_SPxG_matrix_by_selected_feature(set_name, feature_lst, X, y):
    ...
    print(X_filtered.shape, len(y))

    return X_filtered, y

def combine_SPxG_matrix_table(SPxG_matrix_lst, labels, samples_SPID_lst, X_col_name_lst, variant_type_lst):
    ...
    for X, y, row_names_SPID, X_col_name in zip(SPxG_matrix_lst, labels, samples_SPID_lst, X_col_name_lst):
        df = pd.DataFrame(X, columns=X_col_name)
        df.index=row_names_SPID
        df = df.T.join(tmp_df, how="right")
        df = df.T
        df = df.fillna(0)
        combined_SPxG_df_lst.append(df)

    # inter combined data frame and got variant type as first level of column names
    combined_SPxG_df_0 = pd.concat(combined_SPxG_df_lst, keys=variant_type_lst, join="inner", axis=1)
    ...
    # output
    # combined_X = combined_SPxG_df.values
    ...
    combined_samples_SPID = combined_SPxG_df.index
    col_name_w_variant_type = combined_SPxG_df.columns
    
    return combined_SPxG_df, combined_labels, combined_samples_SPID, col_name_w_variant_type
...
class Combined_SPxG_from_variant_type():
    def __init__(self, variant_type_lst, selected_genes, training=True):
        ...
        combined_X, combined_labels, combined_samples_SPID_lst, combined_X_col_name_lst = combine_SPxG_matrix_table(X_lst, y_lst, samples_SPID_lst, X_col_name_lst, variant_type_lst)
